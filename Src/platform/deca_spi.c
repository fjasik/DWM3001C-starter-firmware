...
static uint8_t idatabuf[DATALEN1] = {
    0}; // Never define this inside the Spi read/write
static uint8_t itempbuf[DATALEN1] = {
    0}; // As that will use the stack from the Task, which are not such long!!!!
...
void dwm3001c_spi_init(void) {
    ...
    spi_config->ss_pin =
        NRF_DRV_SPI_PIN_NOT_USED; // pin driven manually, not by the driver
...
void port_set_dw_ic_spi_slowrate(void) {
    ...
    APP_ERROR_CHECK(nrf_drv_spi_init(
        &pgSpiHandler->spi_inst, &pgSpiHandler->spi_config, NULL, NULL));
...
void port_set_dw_ic_spi_fastrate(void) {
    ...
    nrf_gpio_cfg(
        pgSpiHandler->spi_config.sck_pin,
    ...
    nrf_gpio_cfg(
        pgSpiHandler->spi_config.mosi_pin,
        NRF_GPIO_PIN_DIR_OUTPUT,
...
int writetospiwithcrc(
    ...
    uint32_t idatalength =
        ...
        + sizeof(
            crc8); // It cannot be more than 255 in total length (header + body)
    ...
    nrf_drv_spi_transfer(
        &pgSpiHandler->spi_inst, idatabuf, idatalength, itempbuf, idatalength);
