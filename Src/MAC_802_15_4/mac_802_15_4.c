...
void mac_frame_set_pan_ids_and_addresses_802_15_4(
    ...
    uint64_t dest_addr /*,uint16_t src_pan_id*/,
    uint64_t src_addr) {
    ...
    for (cnt = 0; cnt < sizeof(uint64_t); cnt++) // EXTENDED_ADDR
    {
        ...
        src_addr >>= 8;
        MAC_FRAME_DEST_ADDR_ID_802_15_4(mac_frame_ptr, cnt) =
            (uint8_t)(dest_addr);
        dest_addr >>= 8;
    }

    MAC_FRAME_DEST_PAN_ID_802_15_4(mac_frame_ptr, 0) = (uint8_t)dest_pan_id;
    MAC_FRAME_DEST_PAN_ID_802_15_4(mac_frame_ptr, 1) =
        (uint8_t)(dest_pan_id >> 8);
...
void mac_frame_init_mac_frame_ctrl(mac_frame_802_15_4_format_t* mac_frame_ptr) {
    /*
     * Frame control[0]= Data, protected, no more data, no ACK, PEND ID for dest
     * (no for source) Frame control[1]=Has seq num, no IE, dest addr extended,
     * frame ver 2 (IEEE Std 802.15.4), source extended
     */
...
/* Security control -  Security level MIC 16(data conf OFF, data Auth Yes), key
 * determine from key index, has frame cnt, frame cnt gen nonce */
void mac_frame_set_AUX_security_control(
    mac_frame_802_15_4_format_t* mac_frame_ptr) {
    MAC_FRAME_AUX_SECURITY_CTRL_802_15_4(mac_frame_ptr) =
        (AUX_SEC_LEVEL_DATA_CONF_ON_MIC_16 << AUX_SECURITY_LEVEL_SHIFT_VALUE)
        ...
        | (AUX_ASN_IN_NOUNCE_FRAME_CNT_GEN_NONCE << AUX_ASN_IN_NONCE);
}
...
void mac_frame_set_AUX_key_identifier(
    ...
    MAC_FRAME_AUX_KEY_IDENTIFY_802_15_4(mac_frame_ptr) =
        key_index; /*First keys set*/
}

/* Update the frame counter in the MAC frame - AUX security - Frame counter */
void mac_frame_update_aux_frame_cnt(
    mac_frame_802_15_4_format_t* mac_frame_802_15_4_format_ptr,
    uint32_t frame_cnt) {
    uint8_t cnt;

    for (cnt = 0; cnt < AUX_FRAME_CNT_SIZE; cnt++) {
        MAC_FRAME_AUX_FRAME_CNT_802_15_4(mac_frame_802_15_4_format_ptr, cnt) =
            (uint8_t)(frame_cnt);
        frame_cnt >>= 8;
    }
}
...
mac_frame_get_aux_frame_cnt(mac_frame_802_15_4_format_t* mac_frame_ptr) {
    ...
    for (cnt = 0; cnt < AUX_FRAME_CNT_SIZE; cnt++) {
        frame_cnt |=
            (uint32_t)MAC_FRAME_AUX_FRAME_CNT_802_15_4(mac_frame_ptr, cnt)
            << (cnt * 8);
    }
    return frame_cnt;
}
 ...
 * In SS-TWR AES example as specified by IEEE802.15.4 the nonce is of 13-bytes
 * and is made up of Source Address (8), Frame Counter (4) and Nonce Security
 * Level (1)
 *
 * */
void mac_frame_get_nonce(
    mac_frame_802_15_4_format_t* mac_frame_ptr, uint8_t* aes_iv) {
...
/* @fn      rx_aes_802_15_4
 * @brief   Decrypts received frame, the frame type needs to match the structure
 * defined in mac_802_15_4.h - mac_frame_802_15_4_format_t. Note, the register
 * key AES128 should be set before 1st call to this function.
 ...
 * */
aes_results_e rx_aes_802_15_4(
    mac_frame_802_15_4_format_t* mac_frame_ptr,
    uint16_t frame_length,
    dwt_aes_job_t* aes_job,
    uint16_t max_payload,
    dwt_aes_key_t* aes_key_ptr,
    uint64_t exp_src_addr,
    uint64_t exp_dst_addr,
    ...
    if ((frame_length - FCS_LEN) >= aes_job->header_len) {
        ...
        dwt_readrxdata(
            (uint8_t*)MHR_802_15_4_PTR(mac_frame_ptr), aes_job->header_len, 0);
        ...
        // Check if we got a secure frame with the right destination and source
        // addresses
        if ((security_state != SECURITY_STATE_SECURE)
            || (exp_src_addr != src_addr) || (exp_dst_addr != dst_addr))
        {
        ...
        payload_len = frame_length
                    - (aes_job->header_len + aes_job->mic_size
                       + FCS_LEN); /* to get unencrypted payload length subtract
                                      MIC, FCS and MHR lengths */
        ...
        dwt_set_keyreg_128(
            &aes_key_ptr
                [MAC_FRAME_AUX_KEY_IDENTIFY_802_15_4(mac_frame_ptr) - 1]);

        /* perform the decryption job, the unencrypted payload will be stored in
         * aes_job->payload */
 ...
 * @brief   This function checks if the security bit is set in the frame header
 * corresponding security_state_e value.
 *
 * @param   frame pointer
 * @return  security_state_e according to the state
 */
security_state_e
get_security_state(mac_frame_802_15_4_format_t* mac_frame_ptr) {
    if (MAC_FRAME_FRAME_CTRL_802_15_4(mac_frame_ptr, 0)
        & SECURITY_ENABLE_BIT_MASK)
    {
 ...
 * @brief   Copy the source and destination data from the frame header into
 * given destination and source buffers.
 *
 * @param   mac_frame_ptr - pointer to MAC frame
 * @param   src           - pointer to 64-bit integer where the src address will
 * be stored
 * @param   dst           - pointer to 64-bit integer where the dst address will
 * be stored
 * @return  None
 */
void get_src_and_dst_frame_addr(
    mac_frame_802_15_4_format_t* mac_frame_ptr, uint64_t* src, uint64_t* dst) {
