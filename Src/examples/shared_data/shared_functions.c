 ...
 * durations, which is in the magnitude of the DW3XXX TxOutput setting.
 */
...
uint8_t calculate_power_boost(uint16_t frame_duration_us) {
    ...
    // If the frame is longer than the reference duration, then no boost to
    // apply
...
void check_for_status_errors(uint32_t reg, uint32_t* errors) {
    ...
    if (reg & DWT_INT_CPERR_BIT_MASK) {
        ...
        // Get the status for a more detailed error reading of what went wrong
        // with the STS
...
void waitforsysstatus(
    ...
    // If a mask has been passed into the function for the system status
    // register (lower 32-bits)
    if (lo_mask) {
        while (!((lo_result_tmp = dwt_readsysstatuslo()) & (lo_mask))) {
            // If a mask value is set for the system status register (higher
            // 32-bits)
            if (hi_mask) {
                // If mask value for the system status register (higher 32-bits)
                // is found
    ...
    // if only a mask value for the system status register (higher 32-bits) is
    // set
    else if (hi_mask) {
