 ...
 * defined in deca_device_api.h - dwt_test_aes_header_s. Note, register key
 * AES128 should be set before first usage of a function.
 * @param   frame_length-length data that was received, aes_job-holds the params
 * for decryption, payload-payload pointer, payload_load_size-size of receive
 * buffer
 *
 * @return aes_results_e
 * */
aes_results_e rx_aes_802_15_8(
    uint16_t frame_length,
    dwt_aes_job_t* aes_job,
    uint8_t* payload,
    uint16_t payload_load_size,
    ...
    payload_len =
        frame_length
        - (sizeof(header) + aes_job->mic_size
           + FCS_LEN); /* hard-coded MIC size of 16 bytes and 2 bytes of FCS */

    if (payload_len >= 0 && payload_len < payload_load_size) {
        /* Download a max size of a plain text header which we are expecting in
         * the frame. */
         ...
         * In 802.15.8 standard, the 96-bit nonce shall be constructed, using 6
         * bytes of Sender address and 6 bytes of a unique part, called PN.
         * */
        ...
        status =
            dwt_do_aes(aes_job, core_type); // After this command, payload will
